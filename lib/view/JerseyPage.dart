...
Future<void> _placeOrder(
  ...
  try {
    ...
    request.fields['xs'] = (int.tryParse(xs ?? '') ?? 0).toString(); // Handle null or empty
    request.fields['small'] = (int.tryParse(small ?? '') ?? 0).toString(); // Handle null or empty
    request.fields['medium'] = (int.tryParse(medium ?? '') ?? 0).toString(); // Handle null or empty
    request.fields['large'] = (int.tryParse(large ?? '') ?? 0).toString(); // Handle null or empty
    request.fields['xl'] = (int.tryParse(xl ?? '') ?? 0).toString(); // Handle null or empty
    request.fields['xxl'] = (int.tryParse(xxl ?? '') ?? 0).toString(); // Handle null or empty
    request.fields['xxxl'] = (int.tryParse(xxxl ?? '') ?? 0).toString(); // Handle null or empty
