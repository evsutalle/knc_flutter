...
const String uniformEndpoint = '/get_signage.php';
const String orderEndpoint = '/order_signage.php';
...
class SignagePage extends StatefulWidget {
  ...
  const SignagePage({super.key, required this.userId});

  @override
  _SignagePageState createState() => _SignagePageState();
}
...
class _SignagePageState extends State<SignagePage> {
 ...
 @override
  ...
  Future<void> fetchDtfServices() async {
    try {
    final response = await http.get(Uri.parse('$apiBaseUrl$uniformEndpoint'));
      ...
      if (response.statusCode == 200) {
        ...
        _showErrorMessage('Failed to load Signages services. Please try again later.');
      }
    } catch (e) {
  ...
  bool withoutDesign,
    ...
    try {
      ...
      request.fields['signage_id'] = service['signage_id'].toString();
      request.fields['service_name'] = service['service_name'];
      request.fields['size'] = service['size'];
      request.fields['lighted'] = service['lighted'] == 1 ? '1' : '0'; // Lighted field
    ...
    double totalAmount = double.parse(service['price'].toString()) *
        int.parse(quantity);
...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: Text('Signage Services', style: TextStyle(color: Colors.white)),
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 8),
                                // Service Type
                                _buildDetailText('Service Type: ', service['service_name']),
                                SizedBox(height: 8),
                                // Size
                                _buildDetailText('Size: ', service['size']),
                                SizedBox(height: 8),
                                // Lighted
                                _buildDetailText('Lighted: ', service['lighted'] == 1 ? 'YES' : 'NO'),
                                SizedBox(height: 8),
                                // Price
