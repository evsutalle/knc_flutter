...
const String stickerEndpoint = '/get_sticker.php';
const String orderEndpoint = '/order_sticker.php';
...
class StickerPage extends StatefulWidget {
  ...
  const StickerPage({super.key, required this.userId});
...
class _StickerPageState extends State<StickerPage> {
  ...
  Future<void> fetchDtfServices() async {
    try {
    final response = await http.get(Uri.parse('$apiBaseUrl$stickerEndpoint'));
      ...
      if (response.statusCode == 200) {
        ...
        _showErrorMessage('Failed to load Sticker & Labels services. Please try again later.');
      }
    } catch (e) {
  ...
  bool withoutDesign, 
    ...
    try {
      ...
      request.fields['sticker_id'] = service['sticker_id'].toString();
      request.fields['service_name'] = service['service_name'];
      request.fields['material_type'] = service['material_type'];
...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: Text('Sticker & Labels Services', style: TextStyle(color: Colors.white)),
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 8),
                                // Material Type
                                _buildDetailText('Material Type: ', service['material_type']),
                                SizedBox(height: 8),
                                // Size
                                _buildDetailText('Size: ', service['size']),
                                SizedBox(height: 8),
                                // Price per Sq Ft
                                _buildDetailText('Price: ', '₱${service['price_per_sqft']}'),
                                SizedBox(height: 8),
                                // Description
