...
const String souvenirEndpoint = '/get_souvenir.php';
const String orderEndpoint = '/order_souvenir.php';
...
class SouvenirPage extends StatefulWidget {
  ...
  const SouvenirPage({super.key, required this.userId});
...
class _SouvenirPageState extends State<SouvenirPage> {
 ...
 @override
  ...
  Future<void> fetchDtfServices() async {
    try {
    final response = await http.get(Uri.parse('$apiBaseUrl$souvenirEndpoint'));
      ...
      if (response.statusCode == 200) {
        ...
        _showErrorMessage('Failed to load Souvenir services. Please try again later.');
      }
    } catch (e) {
 ...
 void _showOrderModal(BuildContext context, Map<String, dynamic> service) {
  ...
  bool withoutDesign,
    ...
    try {
      ...
      request.fields['souvenir_id'] = service['souvenir_id'].toString();
      request.fields['service_name'] = service['service_name'];
      request.fields['item_type'] = service['item_type'];
      ...
      request.fields['additional_description'] = description;
